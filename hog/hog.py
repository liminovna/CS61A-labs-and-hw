...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    >>> take_turn(2, 0, dice=make_test_dice(4, 5, 1))
    9
    >>> take_turn(3, 0, dice=make_test_dice(4, 6, 1))
    1
    >>> take_turn(0, 2)
    4
    >>> take_turn(0, 0)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += take_turn(strat, score1, dice=dice, goal=goal) + hog_pile(score0, score1)
        else:
            strat = strategy1(score1, score0)
            score1 += take_turn(strat, score0, dice=dice, goal=goal) + hog_pile(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def calculate(score0, score1):
        ...
        if running_high < curr_score:
            print(f"{curr_score} point(s)! That's a record gain for Player {who}!")
            last_score = curr_score 
        return announce_highest(who, running_high, last_score)
    return calculate
    # END PROBLEM 7
