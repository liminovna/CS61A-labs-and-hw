...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_l = [l for l in paragraphs if select(l)]
    if k <= len(new_l)-1:
        return new_l[k]
    return ''
    # END PROBLEM 1
...
def about(topics):
    ...
    # BEGIN PROBLEM 2
    def helper(para):
        para_l = split(remove_punctuation(lower(para)))
        for w in topics:  # [value for value in lst1 if value in lst2]
            if w in para_l:
                return True
        return False
    return helper
...
def accuracy(typed, reference):
    ...
    if len(reference_words) + len(typed_words) == 0:
        ...
        errors = 0
        list_subtr = len(typed_words)-len(reference_words)
        sum_l = list(zip(typed_words, reference_words))
        for pair in sum_l:
            if pair[0] != pair[1]:
                errors += 1
        if list_subtr > 0:
            for extra_w in range(list_subtr):
                errors += 1
        return (len(typed_words)-errors) / len(typed_words) * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    avg_words = len(typed) / 5
    time_in_minutes = elapsed / 60
    return avg_words / time_in_minutes
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    if typed_word not in valid_words:
        res = limit+1
        block = False
        for val_w in valid_words:
            new_res = diff_function(typed_word, val_w, limit)
            if new_res < res:
                res = new_res
                candidate = val_w
            elif new_res == limit and block is False:
                candidate2 = val_w
                block = True
        if res > limit:
            return typed_word
